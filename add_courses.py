...
with open('src/data/curriculum.ts', 'r') as f:
    content = f.read()

# CS306 and CS307 to be inserted after CS305
cs306_307 = """      {
        id: 'CS306',
...
# CS502B and CS502C to be inserted after CS502
cs502bc = """      {
        id: 'CS502B',
...
# Insert cs306_307 after CS305
content = content.replace("whenStuck: 'If a cloud deployment fails: check the logs first (CloudWatch/Cloud Logging). 90% of issues are: wrong IAM permissions, misconfigured security groups, or missing environment variables.'\n      }\n    ]", "whenStuck: 'If a cloud deployment fails: check the logs first (CloudWatch/Cloud Logging). 90% of issues are: wrong IAM permissions, misconfigured security groups, or missing environment variables.'\n      },\n" + cs306_307 + "\n    ]")

# Insert cs502bc after CS502
content = content.replace("whenStuck: 'Getting no callbacks? 90% of the time the problem is your resume or portfolio — not the market. Send your resume to 3 senior engineers you trust for brutally honest feedback.'\n      },\n    ]", "whenStuck: 'Getting no callbacks? 90% of the time the problem is your resume or portfolio — not the market. Send your resume to 3 senior engineers you trust for brutally honest feedback.'\n      },\n" + cs502bc + "\n    ]")

# Update phase 3 and 5 hours, and durations if desired.
# Phase 3 hours: 640 -> 640 + 50 + 40 = 730
content = re.sub(r"(id: 3,\n\s*name: 'Backend & Cloud Engineering',\n\s*duration: ')6-8 months(',\n\s*hours: )640", r"\g<1>7-9 months\g<2>730", content)

# Phase 5 hours: 240 -> 240 + 40 + 20 = 300
content = re.sub(r"(id: 5,\n\s*name: 'Career Preparation & Portfolio',\n\s*duration: ')2-3 months(',\n\s*hours: )240", r"\g<1>3-4 months\g<2>300", content)

# Debugging Masterclass & Ethics & Privacy injections
# CS101 - Week 1-2
content = content.replace("'Weeks 1-2: Python Basics': [\n            'Variables, data types", "'Weeks 1-2: Python Basics': [\n            'Variables, data types", 1) # Keep
content = content.replace("'Weeks 1-2: Python Basics': [", "'Weeks 1-2: Python Basics': [\n            'Debugger Basics: Learn pdb, set breakpoints, step through code',")

# CS101 - Week 4
content = content.replace("'Week 4: Functions': [", "'Week 4: Functions': [\n            'Debugging: Step through loops and function calls to watch variables change',")

# CS101 - Week 6
content = content.replace("'Week 6: OOP': [", "'Week 6: OOP': [\n            'Debugging OOP: Step through code to see object state and understand self',")

# CS201 - Add debugging
content = content.replace("'Week 1: Big-O & Basic Structures': [", "'Week 1: Big-O & Basic Structures': [\n            'Debugging DSA: Use debugger instead of print() for tracking state of algorithms',")

# CS203 - Databases Add Ethics & Privacy
content = content.replace("'Week 1: Relational DBs & SQL': [", "'Week 1: Relational DBs & SQL': [\n            'Ethics & Privacy: Understand what data to store (and what NOT to store)',")
content = content.replace("'Week 3: Advanced SQL & Optimization': [", "'Week 3: Advanced SQL & Optimization': [\n            'Privacy by Design: PII handling, GDPR basics (right to deletion, data access)',")
content = content.replace("'Week 4: NoSQL & Caching': [", "'Week 4: NoSQL & Caching': [\n            'Security: Password hashing (bcrypt), never log passwords or store credit cards in plain text',")

# CS204 - Networks Add Ethics
content = content.replace("'Week 2: Application Layer (HTTP, DNS)': [", "'Week 2: Application Layer (HTTP, DNS)': [\n            'Ethics Context: HTTPS/encryption protects user privacy',")

# CS401 - Debugging Data
content = content.replace("'Week 1: ML Fundamentals': [", "'Week 1: ML Fundamentals': [\n            'Debugging ML: Model not working? Systematically debug data distributions, outliers, leakage',")

# CS403 - MLOps
content = content.replace("'Week 1: ML Lifecycle & Tracking': [", "'Week 1: ML Lifecycle & Tracking': [\n            'Debugging in Production: Monitoring model drift and data validation',")

with open('src/data/curriculum.ts', 'w') as f:
    f.write(content)

print("Done updating curriculum.ts")
